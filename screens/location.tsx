import { View, Image, TextInput, Button, ScrollView, StyleSheet, Text, ViewProps, ActivityIndicator, TouchableOpacity, NativeSyntheticEvent, FlatList } from "react-native";
...
export const LocationList = ({ navigation }: LocationListProps) => {
	...
	return !coords || !profile || !locs ? <ActivityIndicator animating={true} /> :
		<WithNavigation current="locations" navigation={navigation} profile={profile}>
			<FlatList data={locs} renderItem={l => (<TouchableOpacity key={l.item[0].id} onPress={() => { navigation.navigate("LocationDetail", { "id": l.item[0].id }) }}>
				<View>
					<View style={styles.row}>
						<Text>{l.item[0].name}</Text>
						<Text>{l.item[0].description}</Text>
						<Text>由{l.item[1].name}发现</Text>
						<Text>距离:{l.item[4]}</Text>
						<Text>评价:{l.item[5].total / l.item[5].count}</Text>
						{
							l.item[3].map(img => (<Image key={img.id} style={{ width: 100, height: 100 }} source={{ uri: BASE_URL + `/api/upload/${img.id}`, headers: { JWT_TOKEN: profile.token } }} />))
						}
					</View>
					<View style={styles.row}>
